...
export function AppSidebarNav({ title, items }: NavGroupProps) {
...
function checkIsActive(href: string, item: NavItem, mainNav = false) {
  const onlyPath = href.split("?")[0];
  return (
    href === item.url ||
    onlyPath === item.url ||
    !!item?.items?.some((i) => i.url === href || i.url === onlyPath) ||
    (mainNav &&
      onlyPath.split("/")[1] !== "" &&
      onlyPath.split("/")[1] === item?.url?.split("/")[1])
  );
}
